...
class CrossModalGOTFusion(torch.nn.Module):
    """Learn a shared representation with graph optimal-transport guidance.

    This transfers MoST-IG's two-level alignment idea to paired spatial omics:
    Wasserstein distance aligns node distributions, Gromov-Wasserstein distance
    aligns latent graph topology.  To avoid sacrificing biological structure
    for near-perfect pair matching, the learned branch is only a bounded
    residual on top of the normalized arithmetic mean.
    """
    ...
    def __init__(
        ...
        if not 0.0 <= learned_fraction <= 1.0:
            raise ValueError(
                f"learned_fraction must be in [0, 1], got {learned_fraction}."
            )
        ...
        self.shared_norm = torch.nn.LayerNorm(latent_dim)
        ...
        self.output_norm = torch.nn.LayerNorm(latent_dim)

    def adapt(self, z: torch.Tensor, modality: int) -> torch.Tensor:
        """Apply a residual modality adapter and shared normalization."""
        return self.shared_norm(z + self.adapters[modality](z))
    ...
    def forward(
        ...
        if z_first.shape != z_second.shape:
            raise ValueError(
                "GOT fusion requires paired modalities with identical latent shapes; "
                f"got {tuple(z_first.shape)} and {tuple(z_second.shape)}."
            )
        za = self.adapt(z_first, 0)
        zb = self.adapt(z_second, 1)
        gate_features = torch.cat([za, zb, (za - zb).abs(), za * zb], dim=1)
        weights = F.softmax(self.gate(gate_features), dim=1)
        weighted = weights[:, :1] * za + weights[:, 1:] * zb
        correction = torch.tanh(self.correction(torch.cat([za, zb], dim=1)))
        learned = self.output_norm(weighted + self.residual_scale * correction)

                                                                               
                                                                         
        first_unit = F.normalize(z_first, p=2, dim=1)
        second_unit = F.normalize(z_second, p=2, dim=1)
        base_unit = F.normalize(first_unit + second_unit, p=2, dim=1)
        learned_unit = F.normalize(learned, p=2, dim=1)
        fused_unit = F.normalize(
            (1.0 - self.learned_fraction) * base_unit
            + self.learned_fraction * learned_unit,
            p=2,
            dim=1,
        )
                                                                               
        latent_scale = 0.5 * (
            z_first.norm(p=2, dim=1, keepdim=True)
            + z_second.norm(p=2, dim=1, keepdim=True)
        )
        fused = fused_unit * latent_scale
        if return_gate:
    ...
    def structure_loss(
        ...
        """Preserve average within-modality geometry in the fused embedding."""
        n = min(z_first.shape[0], z_second.shape[0], self.max_samples)
        if n < 2:
            return fused.sum() * 0.0
        indices = torch.randperm(z_first.shape[0], device=z_first.device)[:n]
        with torch.no_grad():
            target = 0.5 * (
                self._cosine_distance(z_first[indices], z_first[indices])
                + self._cosine_distance(z_second[indices], z_second[indices])
            )
        observed = self._cosine_distance(fused[indices], fused[indices])
        return F.smooth_l1_loss(observed, target)
    ...
    def _sinkhorn_plan(self, cost: torch.Tensor) -> torch.Tensor:
        """Estimate a balanced entropic OT plan in the log domain."""
        n, m = cost.shape
        log_a = cost.new_full((n,), -np.log(float(n)))
        log_b = cost.new_full((m,), -np.log(float(m)))
        kernel = -cost / self.sinkhorn_epsilon
        log_u = torch.zeros_like(log_a)
        log_v = torch.zeros_like(log_b)
        for _ in range(self.sinkhorn_iterations):
            log_u = log_a - torch.logsumexp(kernel + log_v.unsqueeze(0), dim=1)
            log_v = log_b - torch.logsumexp(kernel + log_u.unsqueeze(1), dim=0)
        return torch.exp(log_u.unsqueeze(1) + kernel + log_v.unsqueeze(0))
    ...
    def alignment_losses(
        ...
        """Return node-WD, fixed-plan GWD and paired contrastive losses."""
        if z_first.ndim != 2 or z_second.ndim != 2:
            raise ValueError("Expected two 2-D latent matrices.")
        if z_first.shape[1] != z_second.shape[1]:
            raise ValueError("Both modalities must use the same latent dimension.")
        n = min(z_first.shape[0], z_second.shape[0], self.max_samples)
        if n < 2:
            zero = (z_first.sum() + z_second.sum()) * 0.0
            return {"wd": zero, "gwd": zero, "contrast": zero}

        paired = z_first.shape[0] == z_second.shape[0]
        if paired:
            idx = torch.randperm(z_first.shape[0], device=z_first.device)[:n]
            first, second = z_first[idx], z_second[idx]
        else:
            idx_first = torch.randperm(z_first.shape[0], device=z_first.device)[:n]
            idx_second = torch.randperm(z_second.shape[0], device=z_second.device)[:n]
            first, second = z_first[idx_first], z_second[idx_second]

        first = self.adapt(first, 0)
        second = self.adapt(second, 1)
        cross_cost = self._cosine_distance(first, second)
        with torch.no_grad():
            plan = self._sinkhorn_plan(cross_cost.detach())
        wd_loss = (plan * cross_cost).sum()

                                                                          
                                                                          
        dist_first = self._cosine_distance(first, first)
        dist_second = self._cosine_distance(second, second)
        row_mass, col_mass = plan.sum(dim=1), plan.sum(dim=0)
        gwd_loss = (
            (dist_first.square() * row_mass[:, None] * row_mass[None, :]).sum()
            + (dist_second.square() * col_mass[:, None] * col_mass[None, :]).sum()
            - 2.0 * torch.trace(dist_first @ plan @ dist_second @ plan.t())
        ).clamp_min(0.0)
        ...
        if paired:
            ...
            contrast_loss = wd_loss * 0.0
        return {"wd": wd_loss, "gwd": gwd_loss, "contrast": contrast_loss}
