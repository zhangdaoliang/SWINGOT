...
def cal_spatial_net(adata,
    ...
    cutoff
        radius cutoff when model='Radius' and the number of nearest neighbors when model='KNN'
    model
    ...
    copy
        ...
        is returned.
    dropEdege
        Randomly drop percent% edges.
    cluster_key : str or None
        Column in adata.obs indicating cell class/type. If provided, keep only edges within the same class.
    ...
    if dropEdge > 0:
        assert dropEdge <= 1.0
        np.random.seed(0)
        all_edges = Spatial_Net.shape[0]
        presv_edge = np.random.choice(all_edges, int(all_edges * (1-dropEdge)), replace=False)
        Spatial_Net = Spatial_Net.iloc[presv_edge,:]
    
    if cluster_key is not None:
        if cluster_key not in adata.obs.columns:
            raise KeyError(f"`cluster_key='{cluster_key}'` not exist in adata.obs.columns")
        cls = adata.obs[cluster_key]
        c1 = Spatial_Net["Cell1"].map(cls)
        c2 = Spatial_Net["Cell2"].map(cls)

        mask = (c1 == c2)

        Spatial_Net = Spatial_Net.loc[mask].copy()
...
def get_gene_annotation(
    ...
    merge_df = pd.concat([
        ...
        pd.DataFrame(gtf).drop(columns=GTF_COLUMNS)                                    
